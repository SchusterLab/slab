...
with program() as binary_decomposition:
    ...
    res_prep_vec = declare(bool, size=num_pi_pulses_m)
    prior_vec = declare(fixed, value=[1.0, 0.0, 0.0, 0.0])  # [0g,0e,1g,1e]
    transmission = declare(fixed, value=[1/16]*16)
    ...
    with for_(n, 0, n < avgs, n + 1):
        ...
        play('CW'*amp(0.0), 'storage', duration=coh_len)
        ########################
        align('storage', 'qubit')
        ...
        with for_(j, 0, j<num_pi_pulses_m,j+1):
            with for_(k, 0, k<prior_vec.length(), k+1):
                assign(prior_vec[k], Math.dot(prior_vec, transmission[k*4:(k+1):4]))
