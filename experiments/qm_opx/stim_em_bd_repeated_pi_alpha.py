...
def stim_em(coh_amp=0.0, coh_len=100, f_state=0, n_pi_m=10, n_pi_n=30, avgs=1000):
...
for ii in range(len(cav_amp)):
    ...
    fock_number = 2
    data = stim_em(coh_amp=coh_amp, coh_len=l/4, f_state=fock_number, n_pi_m=5, n_pi_n=5, avgs=10000)

    path = os.getcwd()

    data_path = os.path.join(path, "data/repeated_pi_pulses/n" + str(fock_number)) + '_reset_self_kerr'
