...
def histogram(a, b, c, d):
    ...
    """Rotate the IQ data"""
    ig_new = a*np.cos(theta) - b*np.sin(theta)
    qg_new = a*np.sin(theta) + b*np.cos(theta)

    ie_new = c*np.cos(theta) - d*np.sin(theta)
    qe_new = c*np.sin(theta) + d*np.cos(theta)
...
if simulation:
    ...
    job = qm.simulate(hist, SimulationConfig(150000))
    samples = job.get_simulated_samples()
    samples.con1.plot()

else:
    """To run the actual experiment"""
    job = qm.execute(hist, duration_limit=0, data_limit=0)
    print("Done")
...
filename = path + "histogram_100MHz_4us_clear_3.h5"
...
fidelity = histogram(Ig, Qg, Ie, Qe)
print("Single shot readout fidility from channel  I  after rotation = %.3f"%fidelity)
