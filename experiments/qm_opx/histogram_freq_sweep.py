...
with program() as histogram:
    ...
    with for_(n, 0, n < avgs, n + 1):
        ...
        with for_(f, rr_IF + f_min, f < f_max + df/2, f + df):
            ...
            play("pi"*amp(0), "qubit")
            align("qubit", "rr")
            measure("long_readout"*amp(1.00), "rr", None, demod.full("long_integW1", I1, 'out1'),
            ...
            # play("pi", "qubit")
            # align("qubit", "rr")
            # measure("long_readout"*amp(1.25), "rr", None, demod.full("long_integW1", I1, 'out1'),
            #         demod.full("long_integW2", Q1, 'out1'),
            #         demod.full("long_integW1", I2, 'out2'), demod.full("long_integW2", Q2, 'out2'))
            #
            # assign(Ie, I1 - Q2)
...
# I_g = np.array(Ig_handle.fetch_all())['value']
# Q_g = np.array(Qg_handle.fetch_all())['value']
#
# I_e = np.array(Ie_handle.fetch_all())['value']
# Q_e = np.array(Qe_handle.fetch_all())['value']

# I_g, Q_g = np.transpose(I_g), np.transpose(Q_g)
# I_e, Q_e = np.transpose(I_e), np.transpose(Q_e)
