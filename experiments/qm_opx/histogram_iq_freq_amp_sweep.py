...
for ii in range(len(f_vec)):
    fid_f = []
    for jj in range(len(amp_vec)):
        ig = Ig[int(jj+ii*len(f_vec))::int(len(f_vec)*len(amp_vec))]
        qg = Qg[int(jj+ii*len(f_vec))::int(len(f_vec)*len(amp_vec))]
        ie = Ie[int(jj+ii*len(f_vec))::int(len(f_vec)*len(amp_vec))]
        qe = Qe[int(jj+ii*len(f_vec))::int(len(f_vec)*len(amp_vec))]
        p = [ig, qg, ie, qe]
        f = hist(p)[0]
        fid_f.append(f)
    fid.append(fid_f)
