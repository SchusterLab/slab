...
## Trial pulse class to see if this fixes readout pulse single square issue
class TwoSquares(Pulse):
    def __init__(self, max_amp, flat_len, ramp_sigma_len, cutoff_sigma, freq, phase, phase_t0=0,delay = 0, dt=None,
                 plot=False):
        self.max_amp = max_amp
        self.flat_len = flat_len
        self.ramp_sigma_len = ramp_sigma_len
    ...
    def get_pulse_array(self):
        ...
        t_flat_end1 = t_flat_start1 + self.flat_len

        t_flat_start2 = self.delay + t_flat_end1 + 2*self.cutoff_sigma * self.ramp_sigma_len
        t_flat_end2 = t_flat_start2 + self.flat_len
        t_end = t_flat_end2 + self.cutoff_sigma * self.ramp_sigma_len
        ...
        pulse_array = self.max_amp * (
            ...
            -1.0 * (self.t_array - (t_flat_end)) ** 2 / (
                ...
                (self.t_array >= self.t0) * (self.t_array < t_flat_start2) * np.exp(
    ...
    def get_length(self):
        return 2*self.flat_len + 4 * self.cutoff_sigma * self.ramp_sigma_len+self.delay
