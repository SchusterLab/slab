...
class Experiment:
    def __init__(self, quantum_device_cfg, experiment_cfg, hardware_cfg,sequences=None, name=None):
        ...
        try:
            # self.fluxbias = [im[flxbias] for flxbias in self.hardware_cfg['fluxbias']]
            ...
            try:
                ...
                fluxbias.initialize()
                time.sleep(1);
                fluxbias.setvoltage(1,1)
            except: print("failure to set channel 1 to 1V on flux bias box")
        except: print( "No dacbox specified in hardware config / failure to connect with im()")
    ...
    def initiate_attenuators(self):
        try:
            for ii, d in enumerate(self.attens):
                #d.set_attenuator(self.quantum_device_cfg['readout']['dig_atten'])  # we now have a readout and a drive digital attenuator!
                digatten = self.quantum_device_cfg['qubit_drive_digital_attenuation']
                d.set_attenuator(digatten)
                print("qubit drive attenuator set to %s"%(digatten))
        except:
