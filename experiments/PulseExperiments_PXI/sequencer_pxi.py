...
# Added by Meg for mixedtones experiments, Oct. 9, 2019
    def complete_mixedtones(self, sequences, plot=True):
        #
        # if sequences.expt_cfg.get('9_calibration', False):
        #     qubit_state = ['g','e','f']
        #
        ...
        #         qubit_id = "1"
        #         if qubit_1_state == 'e':
        #             self.append('charge%s' %qubit_id, sequences.qubit_pi_I[qubit_id])
        #             self.append('charge%s' %qubit_id, sequences.qubit_pi_Q[qubit_id])
        #             self.append('cavity1_I', Idle(time=2150))
        #             self.append('cavity1_Q', Idle(time=2150))
        #                 #self.append_idle_to_time('cavity1_I', 2150)
        #                 #self.append_idle_to_time('cavity1_Q', 2150)
        #                 # below should be...sequences, not self? since it's in the other thing? just write out the sub code of gen_c to make this easier on everyone
        #                 #self.gen_c(sequencer, cavity_id=self.expt_cfg['on_cavities'], amp=0.0001,
        #                            #len=self.pulse_info[qubit_id]['pi_len'] * 4)
        #             sequences.gen_c(self, cavity_id=sequences.expt_cfg['on_cavities'], amp=0.0001, len=sequences.pulse_info[qubit_id]['pi_len']*4)
        #         if qubit_1_state == 'f':
        #             self.append('charge%s' %qubit_id, sequences.qubit_pi_I[qubit_id])
        #             self.append('charge%s' %qubit_id, sequences.qubit_pi_Q[qubit_id])
        #             self.append('cavity1_I', Idle(time=2150))
        ...
        #
        #
        #         #sequences.readout(self, sequences.expt_cfg.get('on_qubits',["1"]))
        #         sequences.readout_pxi(self, sequences.expt_cfg.get('on_qubits', ["1"]))
        #         self.end_sequence()
        #
        # if sequences.expt_cfg.get('pi_calibration', True):
        #     qubit_state = ['g', 'e', 'f']
        #
        #     for qubit_1_state in qubit_state:
        #         self.new_sequence(sequences)
        ...
        #
        #         if qubit_1_state == 'e':
        #             sequences.pi_q(self, qubit_id=qubit_id, phase=0,
        #                            pulse_type=sequences.pulse_info[qubit_id]['pulse_type'])
        #         if qubit_1_state == 'f':
        #             sequences.pi_q(self, qubit_id=qubit_id, phase=0,
        #                            pulse_type=sequences.pulse_info[qubit_id]['pulse_type'])
        #             sequences.pi_q_ef(self, qubit_id=qubit_id, phase=0,
        #                               pulse_type=sequences.pulse_info[qubit_id]['pulse_type'])
        #         sequences.readout_pxi(self, sequences.expt_cfg.get('on_qubits', ["1"]))
        #         self.end_sequence()
    ...
    def complete_mixedtones_2(self, sequences, plot=True):
