...
from slab.instruments.awg.MultimodePulseFluxTwoModesSwapSweepSequences import MultimodeFluxSideBandTwoModesSwapSweepSequence
from numpy import mean, arange


class MultimodeFluxSideBandTwoModesSwapSweepExperiment(Experiment):
    ...
    def go(self,adc):
        self.plotter.clear('MM Flux Sideband Two Modes Swap Sweep Data')
        self.plotter.clear('MM Flux Sideband Two Modes Swap Sweep XY')
        ...
        for ii in arange(max(1, self.mm_flux_sideband_cfg['averages'] / 100)):
            ...
            self.plotter.plot_z('MM Flux Sideband Two Modes Swap Sweep Data', mm_flux_sideband_data.T)
            mm_flux_sideband_avg_data = mean(mm_flux_sideband_data, 1)
            self.plotter.plot_xy('MM Flux Sideband Two Modes Swap Sweep XY', self.pulse_sequence.mm_flux_sideband_pts, mm_flux_sideband_avg_data)
