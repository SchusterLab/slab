...
def fock_prep(f_target=1, phase=0.0):
    ...
    with program() as exp:
        ...
        with for_(n, 0, n < avgs, n + 1):
            ...
            with for_(f, ge_IF[0] + f_min, f < ge_IF[0] + f_max + df/2, f + df):
                ...
                snap_seq(fock_state=f_target, phase=phase)
    ...
    return job
def fock_prep_phase(f_target=1):

    ph_max = 0.1
    ph_min = -0.1
    dph = 0.01
    phase_vec = np.arange(ph_min, ph_max + dph/2, dph)
    ...
    with program() as exp:
        ...
        with for_(n, 0, n < avgs, n + 1):
            ...
            with for_(p, ph_min, p < ph_max + dph/2, p + dph):

                with for_(f, ge_IF[0] + f_min, f < ge_IF[0] + f_max + df/2, f + df):

                    update_frequency("qubit_mode0", ge_IF[0])
                    wait(reset_time// 4, "storage_mode1")# wait for the storage to relax, several T1s
                    snap_seq(fock_state=f_target, phase=p)
                    update_frequency("qubit_mode0", f)
                    align("storage_mode1", "qubit_mode0")
                    play("res_pi", "qubit_mode0")
                    align('qubit_mode0', 'rr')
        ...
        with stream_processing():
            ...
            res_st.boolean_to_int().buffer(len(phase_vec), len(f_vec)).average().save('res')
            I_st.buffer(len(phase_vec), len(f_vec)).average().save('I')
