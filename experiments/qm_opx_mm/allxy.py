...
from configuration_IQ import config, disc_file_opt, ge_IF, disc_file
import numpy as np
    ...
    with for_(n, 0, n < points, n+1):
        ...
        with switch_(r_):
            ...
            with case_(1):
                ...
                save(res, st[1])
            with case_(2):
                frame_rotation(np.pi/2, 'qubit_mode0'); play('pi', 'qubit_mode0'); reset_frame('qubit_mode0')
                frame_rotation(np.pi/2, 'qubit_mode0'); play('pi', 'qubit_mode0'); reset_frame('qubit_mode0')
                align('qubit_mode0', 'rr')
                wait(20, 'rr');discriminator.measure_state("clear", "out1", "out2", res)
                save(res, st[2])
            ...
            with case_(5):
                ...
                save(res, st[5])
            with case_(6):
                frame_rotation(np.pi/2, 'qubit_mode0'); play('pi2', 'qubit_mode0'); reset_frame('qubit_mode0')
                wait(pulse_len_cycle, 'qubit_mode0')
                align('qubit_mode0', 'rr')
                wait(20, 'rr');discriminator.measure_state("clear", "out1", "out2", res)
                save(res, st[6])
