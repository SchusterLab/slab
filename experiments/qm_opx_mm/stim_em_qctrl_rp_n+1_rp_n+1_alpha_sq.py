...
def stim_em(coh_amp=0.0, coh_len=100, f_state=0, n_pi_m=10, n_pi_n=30, avgs=20000, phase=0.0):
    ...
    pulse_filename = './oct_pulses/qctrl/g'+str(f_state)+'.h5'
    ...
    with program() as exp:
        ...
        with for_(n, 0, n < avgs, n + 1):
            ...
            ########################
            """QCTRL OCT pulses to create Fock states"""
            ...
            with if_(bit_sum==0):
                ...
                align('rr', 'qubit_mode0', 'storage_mode1')
                wait(734, "rr") # to match the same time as the time between each repeated pi pulse sequence
                discriminator.measure_state("readout", "out1", "out2", bit, I=I)
                save(I, I_st)
                save(bit, bit_st)
                wait(500, "rr") #same time as the time between each repeated pi pulse sequence
                # update_frequency('qubit_mode0', ge_IF[0] + two_chi_vec[f_state+1])
                align('rr', 'qubit_mode0', 'storage_mode1')
                ########################
                """Repeated pi pulses at n+1"""
...
import json
analysis_params = { 'qubit_params': {'t1':133, 't2':240, 'nth':3e-2},
                    'cavity_params' : {'t1':1.313e3, 'nth':0.0006},
                    'readout_params':{'length':2.2, 'trigger':7.356, 'pi_pulse':3, 'g_inf':0.0468, 'e_inf':0.0454},
                    }

# path = os.getcwd()
data_path = 'S:\\_Data\\20220222 - 3DMM - StimEm - OPX\\data\\stim_em_qctrl_rp_n+1\\20220228\\'
...
camp = [0.0, 0.005, 0.02, 0.04, 0.06, 0.08]
...
for kk in range(4):
    ...
    for ii in range(len(camp)):
        ...
        for f_num in range(5):
            ...
            l = 16 # this should give the same alpha for Gaussian pulse shape as with a square shape of 10
            avgs = 20000
            coh_amp = np.round(camp[ii], 4)
            fock_number = f_num
            job = stim_em(coh_amp=coh_amp, coh_len=l, f_state=fock_number, n_pi_m=3, n_pi_n=30, avgs=avgs, phase=0)
            ...
            print(fock_number, coh_amp, len(Q))

            path = os.getcwd()

            data_path = 'S:\\_Data\\20220222 - 3DMM - StimEm - OPX\\data\\stim_em_qctrl_rp_n+1\\20220228\\n'+ str(fock_number)

            filename = "stim_em_n" + str(fock_number) +"_sq_camp_" + str(coh_amp)+"_len_"+str(l)
