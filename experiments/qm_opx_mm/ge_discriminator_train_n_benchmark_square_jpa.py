...
discriminator = TwoStateDiscriminator(qmm=qmm,
                                      ...
                                      update_tof=False,
                                      rr_qe='rr',
                                      path='ge_disc_params_sq_jpa.npz',
                                      lsb=lsb)
...
with program() as training_program:
    ...
    with for_(n, 0, n < N, n + 1):
        ...
        align("rr", "jpa_pump")
        play('pump_square', 'jpa_pump', duration=long_redout_len//4)
        training_measurement("long_readout", use_opt_weights=use_opt_weights)
        save(I, I_st)
        save(Q, Q_st)
...
with program() as benchmark_readout:
    ...
    with for_(n, 0, n < N, n + 1):
        ...
        align("rr", "jpa_pump")
        play('pump_square', 'jpa_pump', duration=long_redout_len//4)
        discriminator.measure_state("long_readout", "out1", "out2", res, I=I, Q=Q)
        save(res, res_st)
        save(I, I_st)
        save(Q, Q_st)
...
seq_data_file = os.path.join(data_path,
                             get_next_filename(data_path, 'histogram_sq_disc', suffix='.h5'))
print(seq_data_file)
