...
import json
analysis_params = { 'qubit_params': {'t1':118, 't2':117, 'nth':3e-2},
                    'cavity_params' : {'t1':1.352e3, 'nth':0.0006},
                    'readout_params':{'length':3.3, 'trigger':7.356, 'pi_pulse':3, 'g_inf':0.034, 'e_inf':0.0478},
                    }

path = os.getcwd()

data_path = os.path.join(path, "data/stim_em_nobd_nodisc")
...
for f_num in range(4):
    ...
    for ii in range(len(camp)):
        ...
        data_path = os.path.join(path, "data/stim_em_nobd_nodisc/n" + str(fock_number))
