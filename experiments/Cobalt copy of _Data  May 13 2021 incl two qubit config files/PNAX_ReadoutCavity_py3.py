...
nwa = im['PNAX']
# nwa = N5242A("N5242A", address="", timeout=10e3)
print(nwa.get_id())
...
prefix = "PNAX_%s_ReadoutCavityReflection"
...
def two_tone_CW_meg(read_freq,probe_freq_center,span,ifbw = ifbw, read_power = read_power,probe_power = probe_power,sweep_pts= sweep_pts,avgs=avgs,avgs_state=avgs_state):
    # vary_param = "qubit_twotone_CW"
    # print("Swept Parameter: %s" % (vary_param))
    # prefix = "StorageCavityReadout%s_" % vary_param
    fname = get_next_filename(expt_path, prefix, suffix='.h5')
...
#
    ...
    nwa.write('SENSE:FOM:RANGE4:FREQUENCY:START %f' % rfreq)
    nwa.write('SENSE:FOM:RANGE4:FREQUENCY:STOP %f' % rfreq)
    nwa.write('SENSE:FOM:RANGE3:FREQUENCY:START %f' % rfreq)
    nwa.write('SENSE:FOM:RANGE3:FREQUENCY:STOP %f' % rfreq)
...
def single_tone_CW_43(read_freq_center,span,read_power = read_power,ifbw = ifbw, sweep_pts= sweep_pts,avgs=avgs,avgs_state=avgs_state,is_qubitdrive= 0,qfreq= 0,measurement="S43"):
    ...
    #nwa.write('SENSE:FOM:STATE 1')
    nwa.write('SENSE:FOM:STATE 3')
    nwa.write("sense:fom:range1:coupled 0")
    nwa.write("sense:fom:range2:coupled 1")
    nwa.write("sense:fom:range4:coupled 1")
    #nwa.write("sense:fom:range4:coupled 0")
    #nwa.write("sense:fom:range4:coupled 1")
    ...
    nwa.write('SENSE:FOM:RANGE3:FREQUENCY:START %f' % read_freq_start)
    nwa.write('SENSE:FOM:RANGE3:FREQUENCY:STOP %f' % read_freq_stop)
...
##Broad transmission over circulator band with Clara
# set value, not measured, subtract 1 for SN293D
ibias = 0.015

ifbw = 700
fstart = 8.8e9; fstop = 9.0e9;
fcenter = (fstart+fstop)/2; fspan = (fstop-fstart);
prefix = "PNAX_A7B9Transmission_S43_LatticeBand_HEMTCurrent" + str(ibias)
nwa.write(":OUTPUT ON")
single_tone_CW_43(read_freq_center=fcenter, span=fspan, read_power=0, avgs=15, sweep_pts=10001, measurement="S43")
nwa.write(":OUTPUT OFF")



# # Find readout cavity dip at low drive power
# fcenter = 10.570e9; fspan = 20e6;
# prefix = "PNAX_ReadoutCavityReflection"
# nwa.write(":OUTPUT ON")
# single_tone_CW(read_freq_center=fcenter, span=fspan, read_power=-25, avgs=70, sweep_pts=1001, ifbw=500, measurement="S21")
# nwa.write(":OUTPUT OFF")

# Sweep drive power
# powerpoints = np.linspace(25,-25,101)
# fcenter = 10.570e9; fspan = 10e6
# prefix = "PNAX_ReadoutCavityReflection_PowerSweep"
# nwa.write(":OUTPUT ON")
# single_tone_power_sweep(read_power_list=powerpoints, read_freq_center=fcenter, span=fspan, avgs=50, ifbw=1e3, sweep_pts=1001, measurement="S21")
# nwa.write(":OUTPUT OFF")

#========================================= Two tone measurements

# Two tone spec at fixed probe power - trying to find the qubit
# read_freq = 10.56955e9
# read_power = -12
# probe_power = 30
...
# nwa.write(":OUTPUT ON")
# two_tone_CW(read_freq = read_freq, probe_freq_center = fcenter, span = fspan, ifbw = 700, read_power = read_power, probe_power = probe_power, sweep_pts=4001, avgs=60)
# nwa.write(":OUTPUT OFF")

# Two tone qubit spec  - sweeping the probe power, trying to identify the 0->2/2 transition and maybe 1->2 transition
# read_freq = 10.56955e9
# read_power = -12
# powerpoints = np.linspace(35,30,11)
# fstart = 7.3e9; fstop = 7.8e9;
# fcenter = (fstart+fstop)/2; fspan = (fstop-fstart);
# prefix = "PNAX_QubitSpec_PowerSweep"
# nwa.write(":OUTPUT ON")
# two_tone_power_sweep(probe_power_list = powerpoints, read_freq = read_freq, probe_freq_center = fcenter, span = fspan, ifbw = 700, read_power = read_power, sweep_pts=5001, avgs=75)
# nwa.write(":OUTPUT OFF")