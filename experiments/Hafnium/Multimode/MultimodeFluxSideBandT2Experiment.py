...
from slab.instruments.awg.MultimodePulseFluxT2Sequences import MultimodeFluxSideBandT2Sequence
from numpy import mean, arange


class MultimodeFluxSideBandT2Experiment(Experiment):
    def __init__(self, path='', prefix='MM_Flux_Sideband_T2', config_file=None, use_cal=False, **kwargs):
        ...
        self.mm_flux_sideband_cfg = self.cfg['mm_flux_sideband_t2']
        ...
        self.pulse_sequence = MultimodeFluxSideBandT2Sequence(self.cfg['awgs'], self.cfg['mm_flux_sideband_t2'], self.cfg['readout'],self.cfg['pulse_info'][self.pulse_type])
    ...
    def go(self,adc):
        self.plotter.clear('MM Flux Sideband T2 Data')
        self.plotter.clear('MM Flux Sideband T2 XY')
        ...
        for ii in arange(max(1, self.mm_flux_sideband_cfg['averages'] / 100)):
            ...
            self.plotter.plot_z('MM Flux Sideband T2 Data', mm_flux_sideband_data.T)
            mm_flux_sideband_avg_data = mean(mm_flux_sideband_data, 1)
            self.plotter.plot_xy('MM Flux Sideband T2 XY', self.pulse_sequence.mm_flux_sideband_pts, mm_flux_sideband_avg_data)
