...
from slab.instruments.awg.MultimodePulseFluxSweepSequences3 import MultimodeFluxSideBandFluxSweepSequence3
from numpy import mean, arange


class MultimodeFluxSideBandFreqSweepExperiment3(Experiment):
    def __init__(self, path='', prefix='MM_Flux_Sideband_Rabi', config_file=None, use_cal=False, flux_freq = 0, data_file = None, **kwargs):
        Experiment.__init__(self, path=path, prefix=prefix, config_file=config_file, live_plot_enabled=False,**kwargs)

        self.flux_freq = flux_freq
        ...
        self.pulse_sequence = MultimodeFluxSideBandFluxSweepSequence3(self.cfg['awgs'], self.cfg['mm_flux_sideband'], self.cfg['readout'],self.cfg['pulse_info'][self.pulse_type], self.flux_freq)
    ...
    def go(self,adc):
        ...
        for ii in arange(max(1, self.cfg['mm_flux_sideband']['averages'] / 100)):
        ...
        with self.datafile(data_file=self.data_file) as f:
