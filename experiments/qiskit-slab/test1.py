...
class QiskitPulseProgram(ASM_Program):
    def __init__(self):
        super().__init__(self)
    #ENDDEF
#ENDCLASS
...
def rabi():
    ...
    # acquire pulse (instruction)
    acq_pulse = pulse.acquire(meas_duration, acq_chan, MemorySlot(0))
    ...
    for drive_amp in drive_amps:
        ...
        schedule += Play(rabi_pulse, drive_chan)
        meas_start = schedule.duration
        schedule += Play(meas_pulse, meas_chan) << meas_start
        schedule += acq_pulse << meas_start
    ...
    rabi_experiment_program = assemble(
        ...
        schedule_los=[{drive_chan: center_frequency_Hz}] * num_rabi_points
    )
    ...
    for expt_config in qobj_dict["experiments"]:
        pulses = pulses_from_exptconfig(expt_config, qobj_dict)
        
        # write program
        program = QiskitPulseProgram()
        program.synci(1000)
        program.end()
