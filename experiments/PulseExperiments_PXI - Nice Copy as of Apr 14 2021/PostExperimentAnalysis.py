...
#        t = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))]/1e3
    ...
    def cavity_drive_rabi_freq_scan_vstime_mixedtones(self):
        expt_cfg2 = self.experiment_cfg[self.exptname]

        experiment_name = 'cavity_driVe_rabi_freq_scan_vstime_mixedtones'
    ...
    def cavity_drive_sideband_geramsey(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        ramsey_freq = expt_cfg['ramsey_freq']
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
    ...
    def cavity_drive_chi_dressing_calibration(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        ramsey_freq = expt_cfg['ramsey_freq']
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
...
#        print("Qubit frequency = ", p[2], "GHz")
 ...
 #      print("Estimated pi pulse time: ", 1 / (sqrt(2) * 2 * p[3]), 'ns')
    ...
    def cavity_drive_rabi_freq_scan_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        nu_q = self.quantum_device_cfg['qubit']["1"]["freq"]+ self.quantum_device_cfg['cavity']["1"]["f0g1Delta"]
    ...
    def cavity_drive_rabi_mixedtones_withf0g1_fromfreqscan(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        nu_q = self.quantum_device_cfg['qubit']["1"]["freq"] + self.quantum_device_cfg['cavity']["1"]["f0g1Delta"]
        ...
        # need to modify this given that it's not really a freq scan...
       # f = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(self.I))] + nu_q
        ...
        if self.show:
            ...
            p = fitlor(f, eval('self.' + self.P), showfit=False)
            # This is a weird mishmash of cavity_drive_rabi_freq_scan_mixedtones and cavity_drive_rabi_twotone
            # Hopefully it will work
            # This whole experiment is really supposed to be part of a sequential experiment anyways
    ...
    def resonator_spectroscopy_power_sweep(self):
        print("No post expt analysis, plot this in Jupyter or something")

    def resonator_spectroscopy_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        I = self.I.flatten() - mean(self.I.flatten())
        Q = self.Q.flatten() - mean(self.Q.flatten())
        f = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(I))]
    ...
    def cavity_drive_resonator_spectroscopy_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        I = self.I.flatten() - mean(self.I.flatten())
        Q = self.Q.flatten() - mean(self.Q.flatten())
        f = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(I))]
    ...
    def cavity_drive_pulse_probe_iq_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']+1.2
    ...
    def cavity_drive_pulse_probe_ef_iq_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
    ...
    def cavity_drive_ramsey_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        ramsey_freq = expt_cfg['ramsey_freq']
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
    ...
    def cavity_drive_ef_ramsey_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        ramsey_freq = expt_cfg['ramsey_freq']
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
        alpha = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['anharmonicity']
    ...
    def cavity_drive_echo_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        ramsey_freq = expt_cfg['ramsey_freq']
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
    ...
    def amplitude_rabi(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        P = eval('self.'+self.P)
        a = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))]
        t = arange(expt_cfg['starttime'], expt_cfg['stoptime'], expt_cfg['steptime'])[:(len(P))]
        # if self.show:

        fig = plt.figure(figsize=(14, 7))
        ax = fig.add_subplot(111, title=self.exptname)
        ax.plot(a, P, 'o-', label=self.P)
        ax.set_xlabel('Amp (fractional)')
        ax.set_ylabel(self.P)
        ax.legend()
        #p = fitdecaysin(a[2:], P[2:], showfit=True)
        plt.show()
    ...
    def amplitude_time_rabi(self):
        ...
        P = eval('self.'+self.P)
        a = arange(expt_cfg2['start'], expt_cfg2['stop'], expt_cfg2['step'])[:(len(P))]
        t = arange(expt_cfg2['starttime'], expt_cfg2['stoptime'], expt_cfg2['steptime'])[:(len(P))]
        # if self.show:
    ...
    def cavity_drive_rabi_twotone(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        P = eval('self.'+self.P)
        t = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))]
        amp = expt_cfg['amp']
    ...
    def cavity_drive_t1_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        P = eval('self.'+self.P)
        t = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))]/1e3
    ...
    def cavity_drive_ef_rabi_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        P = eval('self.' + self.P)
        t = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))]
        amp = expt_cfg['amp']
    ...
    def cavity_drive_ef_rabi_testing_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        P = eval('self.' + self.P)
        t = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))]
        amp = expt_cfg['amp']
    ...
    def cavity_drive_histogram_mixedtones(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        ran = self.hardware_cfg['awg_info']['keysight_pxi']['m3102_vpp_range']
        readout_length = self.quantum_device_cfg['readout']['length']
        window = self.quantum_device_cfg['readout']['window']
    ...
    def cavity_drive_direct_spectroscopy(self):
        ...
        P = eval('self.' + self.P)
        # MGP changed the actual experiment code in January 2020 so we're looking at g vs. e occupation, pi  calibrated.
        # So everything is just a line at zero if you remove cavity drive pulses and just look at the output, regardless of qubit pulse length
        #self.I = self.I - mean(self.I)
        #self.Q = self.Q - mean(self.Q)

        #f = arange(expt_cfg['start'], expt_cfg['stop'], expt_cfg['step'])[:(len(P))] + nu_c
    ...
    def cavity_drive_direct_spectroscopy_freq_sweep(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        nu_c = self.quantum_device_cfg['cavity'][expt_cfg['on_cavities'][0]]['freq']
    ...
    def cavity_drive_pulse_probe_iq_signalcore(self):
        expt_cfg = self.experiment_cfg[self.exptname]
        nu_q = self.quantum_device_cfg['qubit'][expt_cfg['on_qubits'][0]]['freq']
