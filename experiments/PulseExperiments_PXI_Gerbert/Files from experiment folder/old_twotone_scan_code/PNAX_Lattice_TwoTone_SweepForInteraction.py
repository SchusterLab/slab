...
if isPNAX:
    nwa = im['PNAX2']
    #nwa = N5242A("N5242A", address="192.168.14.181", timeout=10e3)
...
def flux_map_onetone(qubitInd, fluxInd, fluxInd2,offset):
    ...
    numpts = [41]*8 #nb of points in voltage

    # Convert to volts assuming 50 ohms impedance
    diag_offset = [0,0,0,0,0,0,-3.3,0.0]
    diag_range = [0,0,0,0,0,0,0.3,0] #max magnitude of Voltage
    ...
    read_power = [ -70.0]*8 #45
    read_freq_center = [6.197e9,6.298e9,6.4146e9,6.528e9,6.639e9,6.754e9,6.863e9,6.9766e9]
    bw=10e6
    read_freq_start = []
    read_freq_stop = []
    ...
    drive_power = [-40.0]*8 #30

    drive_freq_start = [5.75e9]*8
    drive_freq_stop = [5.95e9]*8
    ...
    if isTakeData:
        ...
        for ii,pt_target in enumerate(all_f_vary_pts):
            ...
            if YOKO == 1:
                ...
                dac.ramp3(fluxInd2+1,offset,step = step,speed=rampspeed)
            time.sleep(2)
            ...
            if phasedetect == 1:
                ...
                freq_peak = freq_peak + 800e3# 6.298e9
                # if ii==0:
...
######
# for ii in [6]:
#     for jj in np.linspace(-6,6,7):
#         flux_map_onetone(qubitInd=ii, fluxInd=ii, fluxInd2=7, offset=jj)
#
for ii in [6]:
    flux_map_onetone(qubitInd=ii, fluxInd=ii, fluxInd2=7, offset=-2)
