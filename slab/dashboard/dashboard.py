...
class DataTree(keydefaultdict):
    def __init__(self, gui, name=None, parentpath=()):
        ...
        self.gui = gui
        keydefaultdict.__init__(self,
             lambda k: DataTree(self.gui, name=k, parentpath=self.path))
...
class DataTreeLeaf(object):
    def __init__(self, path, rank=None, data=(), save=True, plot=True,
...
class DataManager(gui.BackgroundObject):
    ...
    def get_leaf(self, path, rank=None, **initargs):
        ...
        if path[-1] not in group:
            ...
            leaf = DataTreeLeaf(path, rank, **initargs)
            group[path[-1]] = leaf
...
class SlabFileRemote:
    def __init__(self, filename=None, manager=None, context=(), **ignoredargs):
        ...
        if context == () and filename is None:
