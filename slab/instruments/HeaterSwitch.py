...
class HeaterSwitch(WebInstrument):
    ...
    def get_switch(self, num):
        if num in (0, 1, 2, 3, 4):
            return not bool(int(ur.urlopen(self.address + '/Q' + str(num)).read()[3]))
        else:
    ...
    def set_switch(self, num, state):
        if num in (1, 2, 3, 4):
            if state.lower() == 'on':
                return ur.urlopen(self.address + '/S' + str(num) + '0').read()[3]
            elif state.lower() == 'off':
                return ur.urlopen(self.address + '/S' + str(num) + '1').read()[3]
            else:
    ...
    def reset_switch(self):
        return ur.urlopen(self.address + '/R').read()[3]
