...
class KeysightSingleQubit:
    ...
    def __init__(self, experiment_cfg, hardware_cfg, quantum_device_cfg, sequences, name, save_path=r"C:\Users\slab\Documents\Data",
        ...
        for qubit_id in self.on_qubits:
            num_expt = sequences['readout%s' % qubit_id].shape[0]
        ...
        # HACK TO ADD GLOBAL DELAY ON ALL LINES
        # could rename but lazy
        self.tek2_trigger_delay = self.tek2_trigger_delay + self.global_trig_delay_fromawg
        ...
        # added 4/21, idk if works
        if self.two_qubits:
            self.data_1,self.data_2 = np.zeros((self.num_expt, self.DIG_sampl_record)),np.zeros((self.num_expt, self.DIG_sampl_record))
            self.data_3, self.data_4 = np.zeros((self.num_expt, self.DIG_sampl_record)), np.zeros((self.num_expt, self.DIG_sampl_record))
        else:
            ...
            if self.on_qubits == "2":
                ...
                print('You have an issue reconciling the two_qubits name option and on_qubits cobfig option')
    ...
    def configureChannels(self, hardware_cfg, experiment_cfg, name):
        ...
        self.m_8_ch_1.configure(amplitude=amp_mark[0], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        self.m_8_ch_2.configure(amplitude=amp_mark[1], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        self.m_8_ch_4.configure(amplitude=amp_mark[3], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        self.m_8_ch_3.configure(amplitude=amp_mark[2], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        ...
        self.m_9_ch_1.configure(amplitude=amp_mark[0], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        self.m_9_ch_2.configure(amplitude=amp_mark[1], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        self.m_9_ch_4.configure(amplitude=amp_mark[3], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        self.m_9_ch_3.configure(amplitude=amp_mark[2], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        # self.m_ch_4.configure(amplitude=amp_mark[3], trigger_source=SD1.SD_TriggerModes.EXTTRIG)
        # print("tried marker 4")
    ...
    def loadAndQueueWaveforms(self,sequences):
       ...
       # newname=s
        ...
        # "waveform_channels": ["charge1_I", "charge1_Q", "readout", "readout_trig", "tek2_trig", "cavity1_I","cavity1_Q"]
        elif self.on_qubits == "1":
            ...
            print("Error naming waveforms in keysight_pxi_load. You may need to add two_qubits to sequence name.")
        ...
        if self.two_qubits:
            if len(waveforms_A_I) != len(waveforms_A_Q) or len(waveforms_A_I) != len(markers_readout) or len(waveforms_A_I) != len(
                readout_A):
                raise TypeError("Not all waveform A lists are the same length")
            if len(waveforms_B_I) != len(waveforms_B_Q) or len(waveforms_B_I) != len(markers_readout) or len(waveforms_B_I) != len(
                readout_B):
                raise TypeError("Not all waveform B lists are the same length")
            # make duplicate markers readout if this gets hinky
            if len(waveforms_A_I) != len(waveforms_B_I) or len(waveforms_A_Q) != len(waveforms_B_Q) or len(readout_A) != len(
                readout_B):
        ...
        if self.two_qubits:
            ...
            qubit_A_marker_dsp = self.generatemarkers(waveforms_A_I, resample=True, conv_width=self.lo_delay,trig_delay=self.trig_delay)
            qubit_B_marker = self.generatemarkers(waveforms_B_I)
            qubit_B_marker_dsp = self.generatemarkers(waveforms_B_I, resample=True, conv_width=self.lo_delay,trig_delay=self.trig_delay)
            ...
            if self.prep_cavity_drive:
                cavity_A_marker_dsp = self.generatemarkers(cavity_A_I, resample=True,conv_width=self.lo_delay,trig_delay=self.trig_delay)
                cavity_B_marker_dsp = self.generatemarkers(cavity_B_I, resample=True, conv_width=self.lo_delay,trig_delay=self.trig_delay)
            ...
            readout_marker_dsp = self.generatemarkers(readout, rresample=True, trig_delay=self.trig_delay)
        ...
        if self.two_qubits:
            for i in tqdm(range(len(waveforms_A_I))):
                ...
                wave_A_I.queue(self.AWG_A_ch_1, trigger_mode=SD1.SD_TriggerModes.EXTTRIG, delay=self.tek2_trigger_delay + self.iq_line_delay,
                             cycles=1, prescaler=0)
                wave_A_Q.queue(self.AWG_A_ch_2ch_2, trigger_mode=SD1.SD_TriggerModes.EXTTRIG, delay=self.tek2_trigger_delay + self.iq_line_delay,
                             cycles=1, prescaler=0)
                wave_B_I.queue(self.AWG_B_ch_1, trigger_mode=SD1.SD_TriggerModes.EXTTRIG, delay=self.tek2_trigger_delay + self.iq_line_delay,
                               cycles=1, prescaler=0)
                wave_B_Q.queue(self.AWG_B_ch_2, trigger_mode=SD1.SD_TriggerModes.EXTTRIG, delay=self.tek2_trigger_delay + self.iq_line_delay,
                               cycles=1, prescaler=0)
                if self.prep_cavity_drive:
                    wave_cavity_A_I.queue(self.AWG_A_ch_3, trigger_mode=SD1.SD_TriggerModes.EXTTRIG,
                                        delay=self.tek2_trigger_delay + self.iq_line_delay, cycles=1, prescaler=0)
                ...
                self.AWG_A_module.AWGqueueMarkerConfig(nAWG=1, markerMode=1, trgPXImask=0b11111111, trgIOmask=0, value=1,
                ...
                m_A_qubit_dsp.queue(self.m_8_ch_1, trigger_mode=SD1.SD_TriggerModes.EXTTRIG, delay=self.tek2_trigger_delay,
                                  cycles=1, prescaler=0)
                m_A_readout_dsp.queue(self.m_8_ch_2, trigger_mode=SD1.SD_TriggerModes.EXTTRIG,
                                    delay=self.tek2_trigger_delay, cycles=1, prescaler=0)
                m_B_qubit_dsp.queue(self.m_8_ch_3, trigger_mode=SD1.SD_TriggerModes.EXTTRIG,
                                    delay=self.tek2_trigger_delay,
                                    cycles=1, prescaler=0)
                ...
                card_trig.queue(self.m_9_ch_3, trigger_mode=SD1.SD_TriggerModes.EXTTRIG,
                                delay=int(self.card_delay / 100) + self.tek2_trigger_delay + self.iq_line_delay, cycles=1, prescaler=0)
            ...
            for i in tqdm(range(len(waveforms_I))):
                ...
                # Queue the waveforms. Want to set trigger mode to SWHVITRIG to trigger from computer.
                    wave_I.queue(self.AWG_A_ch_1, trigger_mode=SD1.SD_TriggerModes.EXTTRIG,
                                     delay=self.tek2_trigger_delay + self.iq_line_delay,
                                     cycles=1, prescaler=0)
                    ...
                    m_qubit_dsp.queue(self.m_8_ch_1, trigger_mode=SD1.SD_TriggerModes.EXTTRIG, delay=self.tek2_trigger_delay,
                                  cycles=1, prescaler=0)
    ...
    def run(self):
        ...
        try:
            ...
            if self.two_qubits:
                ...
                self.DIG_ch_4.clear()
                self.DIG_ch_4.start()
                # print("cleared digitizer channels 3 and 4 and started")
            else:
                if self.on_qubits == "1":
                    self.DIG_ch_1.clear()
                    self.DIG_ch_1.start()
...
# THIS IS HORRIBLY VERBOSE, CAN I STICK MORE STUFF IN LOOPS?
