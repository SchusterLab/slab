...
class AlazarConfig1():
    _form_fields_ = [("samplesPerRecord", "Samples", 2048),
                     ...
                     ("clock_source", "Clock Source",
                      ["internal", ("internal", "internal"), ("reference", "reference"), ("60 MHz", "60 MHz"),
                       ("1 GHz", "1 GHz")]),
                     ("clock_edge", "Clock Edge", ["rising", ("rising", "Rising"), ("falling", "Falling")]),
                     ("sample_rate", "Sample Rate",
                      [1000, (1, "1 kHz"), (2, "2 kHz"), (5, "5 kHz"), (10, "10 kHz"), (20, "20 kHz"), (1000, "1 MHz"),
                       (2000, "2 MHz"), (5000, "5 MHz"), (10000, "10 MHz"), (20000, "20 MHz"), (50000, "50 MHz"),
                       (100000, "100 MHz"), (250000, "250 MHz"), (500000, "500 MHz"), (1000000, "1 GHz")]),
                     ("trigger_source1", "Trigger Source 1",
                      ["CH_A", ("CH_A", "Channel 1"), ("CH_B", "Channel 2"), ("external", "External"),
                       ("disabled", "Disabled")]),
                     ("trigger_edge1", "Trigger Edge 1", ["rising", ("rising", "Rising"), ("falling", "Falling")]),
                     ("trigger_level1", "Trigger Level 1", 0),
                     ("trigger_source2", "Trigger Source 2",
                      ["disabled", ("CH_A", "Channel 1"), ("CH_B", "Channel 2"), ("external", "External"),
                       ("disabled", "Disabled")]),
...
class Alazar():
    ...
    def configure(self, config=None):
        ...
        if self.config.recordsPerAcquisition < self.config.recordsPerBuffer:
            raise ValueError("recordsPerAcquisition: %d < recordsPerBuffer: %d" % (
            self.config.recordsPerAcquisition, self.config.recordsPerBuffer))
    ...
    def configure_trigger(self, source=None, source2=None, edge=None, edge2=None,
        ...
        print('Z', op, AlazarConstants.trigger_engine_1, AlazarConstants.trigger_source[self.config.trigger_source1],
              AlazarConstants.trigger_edge[self.config.trigger_edge1],
              U32(int_linterp(self.config.trigger_level1, -100, 100, 0, 255)), AlazarConstants.trigger_engine_2,
    ...
    def acquire_avg_data(self, excise=None):
        ...
        while (buffersCompleted < buffersPerAcquisition):
            ...
            for n in range(self.config.recordsPerBuffer):
                if self.config.ch1_enabled: avg_data1 += self.arrs[buf_idx][(n * self.config.samplesPerRecord):(
                            (n + 1) * self.config.samplesPerRecord)]
                if self.config.ch2_enabled: avg_data2 += self.arrs[buf_idx][(
                                                                                        n + self.config.recordsPerBuffer) * self.config.samplesPerRecord:(
                                                                                                                                                                     n + self.config.recordsPerBuffer + 1) * self.config.samplesPerRecord]
    ...
    def acquire_data_by_record(self, prep_function=None, start_function=None, excise=None):
        ...
        while (buffersCompleted < buffersPerAcquisition):
            ...
            ret = self.Az.AlazarPostAsyncBuffer(self.handle, self.bufs[buf_idx].addr,
                                                U32(self.bufs[buf_idx].size_bytes))
            if ret != 512:
                print("Abort AsyncRead, PostAsyncBuffer: ", ret_to_str(ret, self.Az))
                ret = self.Az.AlazarAbortAsyncRead(self.handle)
                break
    ...
    def acquire_avg_data_by_record(self, prep_function=None, start_function=None, excise=None):
        ...
        if num_chs == 2:
            avg_data1, avg_data2 = avg_data.reshape(
                (num_chs, self.config.recordsPerBuffer, self.config.samplesPerRecord))
        else:
        ...
        while (buffersCompleted < buffersPerAcquisition):
            ...
            for n in range(self.config.recordsPerBuffer):
                for ch in range(num_chs):
                    ss_record = np.mean(self.arrs[buf_idx][
    ...
    def acquire_singleshot_heterodyne_multitone_data(self, IFreqList, prep_function=None, start_function=None,
                                                     excise=None, save_raw_data=False):
    ...
    # this takes single shot data, returning cos/sin at multiople freqs in IFreqList
    def acquire_singleshot_heterodyne_multitone_data_2(self, IFreqList, prep_function=None, start_function=None,
                                                       excise=None, save_raw_data=False):
    ...
    def acquire_singleshot_heterodyne_multitone_data_phase_ref(self, IFreqList, hetero_phase_ref_freq,
    ...
    def acquire_singleshot_data2(self, prep_function=None, start_function=None, excise=None):
        ...
        while (buffersCompleted < buffersPerAcquisition):
            ...
            for n in range(self.config.recordsPerBuffer):
                if self.config.ch1_enabled:
                    single_data1[recordsCompleted] = self.arrs[buf_idx][n * self.config.samplesPerRecord:(
                                                                                                                     n + 1) * self.config.samplesPerRecord]

                if self.config.ch2_enabled:
                    single_data2[recordsCompleted] = self.arrs[buf_idx][
                                                     (n + self.config.recordsPerBuffer) * self.config.samplesPerRecord:(
                                                                                                                                   n + self.config.recordsPerBuffer + 1) * self.config.samplesPerRecord]
    ...
    def acquire_phase_compensated_quadratures(self, IFreq, excise=None):
        ...
        while (buffersCompleted < buffersPerAcquisition):
            ...
            for n in range(self.config.recordsPerBuffer):
                ...
                o_ch2_pts = self.arrs[buf_idx][(n + self.config.recordsPerBuffer) * self.config.samplesPerRecord:(
                                                                                                                             n + self.config.recordsPerBuffer + 1) * self.config.samplesPerRecord]
                ...
                # avg_cos += cos_quad1
                # avg_sin += sin_quad1
                avg_cos += cos_quad1 * np.cos(phase_array[rec_no] - phase_array[0]) + sin_quad1 * np.sin(
                    phase_array[rec_no] - phase_array[0])
                avg_sin += sin_quad1 * np.cos(phase_array[rec_no] - phase_array[0]) - cos_quad1 * np.sin(
                    phase_array[rec_no] - phase_array[0])
                rec_no += 1
                avg_data1 += ch1_pts
                avg_data2 += ch2_pts
        ...
        ret = self.Az.AlazarAbortAsyncRead(self.handle)
        if excise is not None:
            return tpts[excise[0]:excise[1]], avg_data1[excise[0]:excise[1]], avg_data2[
                                                                              excise[0]:excise[1]], phase_array
        else:
    ...
    def acquire_cavity_ringdown_data(self, excise=None, frequency_window=None):
        ...
        while (buffersCompleted < buffersPerAcquisition):
            ...
            if ret != 512:
                ...
                ret = self.Az.AlazarAbortAsyncRead(self.handle)
            for n in range(self.config.recordsPerBuffer):
                clipped_psd = np.absolute(fft(self.arrs[buf_idx][n * self.config.samplesPerRecord + excise[
                    0]:n * self.config.samplesPerRecord + excise[1]])[fex[0]:fex[1]])
                ...
                f0 = (x1 ** 2 * y1 * (y2 - y3) + x3 ** 2 * (y1 - y2) * y3 + x2 ** 2 * y2 * (-y1 + y3)) / (
                            2. * (x1 * y1 * (y2 - y3) + x3 * (y1 - y2) * y3 + x2 * y2 * (-y1 + y3)))
                kappa = np.sqrt(-(((x1 - x2) ** 4 * y1 ** 2 * y2 ** 2 - 2 * (x1 - x2) ** 2 * y1 * y2 * (
                            (x1 - x3) ** 2 * y1 + (x2 - x3) ** 2 * y2) * y3 + (
                                               (x1 - x3) ** 2 * y1 - (x2 - x3) ** 2 * y2) ** 2 * y3 ** 2) / (
                                              x1 * y1 * (y2 - y3) + x3 * (y1 - y2) * y3 + x2 * y2 * (-y1 + y3)) ** 2))
                f0_data[(buffersCompleted - 1) * self.config.recordsPerBuffer + n] = f0  # freqs2[maxloc]
                kappa_data[
                    (buffersCompleted - 1) * self.config.recordsPerBuffer + n] = kappa  # 2.*(freqs2[i]-freqs2[maxloc])
            # plot(self.arrs[buf_idx])
        ...
        if DEBUGALAZAR: print("Acquisition finished.")
        print("|")
        print("buffersCompleted: %d, self.config.recordsPerAcquisition: %d" % (
        buffersCompleted, self.config.recordsPerAcquisition))
    ...
    def acquire_homodyne_single_shot(self, frequency):
        ...
        proc_fun = lambda buf_arr: [(np.sum(sin_arr * arr), np.sum(cos_arr * arr)) for arr in
                                    np.split(buf_arr, records_per_buffer)]
