...
from awg import write_Tek5014_file, write_Tek70001_waveform_file
...
class PulseSequence:
    ...
    def write_Tek5014_sequence(self, awg, file_prefix, upload=False):
        ...
        markers=[self.waveforms[marker['name']] for marker in awg['markers']]
        write_Tek5014_file(waveforms,markers,file_prefix+'.awg',self.name)

        if upload:
            im = InstrumentManager()
            im[awg['name']].pre_load()
            im[awg['name']].load_sequence_file(file_prefix+'.awg')
