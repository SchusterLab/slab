...
class MultimodeFluxSideBandFluxSweepSequence3(PulseSequence):
    def __init__(self, awg_info, mm_flux_sideband_cfg, readout_cfg, pulse_cfg, flux_freq):
        """
        ...
        max_flux_pulse_width = max(self.mm_flux_sideband_pts) + 30
    ...
    def build_sequence(self):
        ...
        for ii, d in enumerate(self.mm_flux_sideband_pts):
            ...
            flux_a = 0.25
            flux_freq = self.flux_freq *1e9

            if True:
                flux_flat = self.flux_w
                self.waveforms['qubit 1 flux'][ii] = ap2.sideband(ftpts,
                                     ap2.square(ftpts, flux_a, 15 , flux_flat,5), np.zeros(len(ftpts)),
