...
class Sequencer:
    ...
    def sync_channels_time(self, channels):
        ...
        buffer_time = 0*5 #add 5ns buffer time everytime sync
    ...
    def complete(self, sequences, plot=True):
        if sequences.expt_cfg.get('9_calibration', False):
            ...
            self.new_sequence(sequences)
            sequences.readout(self, sequences.expt_cfg.get('calibration_qubit',["1"]))
            self.end_sequence()


            self.new_sequence(sequences)
            charge_port = sequences.charge_port
            for qubit_id in sequences.expt_cfg.get('calibration_qubit',["1"]):
    ...
    def plot_sequences(self):
        ...
        for sequence in self.multiple_sequences[::1]: #21
...
def testing_function():
    ...
    awg_info = {'m8195a': {'dt': 1. / 32., 'min_increment': 16, 'min_samples': 128, 'time_delay': 110},
