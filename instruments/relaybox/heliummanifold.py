...
class HeliumManifold(RelayBox):
    ...
    def get_pressure(self, avgs=1):
        ...
        for avg in range(avgs):
            pressure = pressure * float(avg) / (avg + 1) + self.vacuum_offset + float(
                self.get_analog_input(self.pressure_port)) / self.atm_level / (avg + 1.)
    ...
    def get_manifold_status(self):
        ...
        self.get_pressure()
        status_str = "gas: %s / Pump: %s / cryostat: %s / Pressure: %f bar" % (
        str(self.gas_state), str(self.pump_state), str(self.cryostat_state), self.pressure)
    ...
    def set_cryostat(self, state=False, override=False):
        if self.DEBUG_HELIUM_MANIFOLD_VERBOSE:
            print "Set cryostat: %s\r" % str(state)
            sys.stdout.flush()
        if not state or override:
    ...
    def wait_for_pressure_rise(self, threshold, timeout=None):
        ...
        while not done:
            if self.DEBUG_HELIUM_MANIFOLD_VERBOSE:
                print str(self.get_pressure()) + '\r'
                sys.stdout.flush()
            if self.get_pressure() > threshold: done = True
            if timeout is not None:
                if time.time() - start_time > timeout:
                    done = True
                    print "HeManifold Pressure timeout final P=%f bar\r" % self.get_pressure()
                    sys.stdout.flush()
        self.get_manifold_status()
    ...
    def pump_manifold(self, min_time=0, timeout=None):
        if self.DEBUG_HELIUM_MANIFOLD:
            print "Pump manifold.\r"
            sys.stdout.flush()
        self.set_gas(False)
    ...
    def fill_manifold(self, fill_level=0.20, timeout=None):
        if self.DEBUG_HELIUM_MANIFOLD:
            print "Fill manifold to %f bar.\r" % (fill_level)
            sys.stdout.flush()
        self.set_cryostat(False)
        self.set_pump(False)
        self.set_gas(True)
    ...
    def clean_manifold(self, n=1, min_time=0, timeout=None):
        if self.DEBUG_HELIUM_MANIFOLD:
            print "Clean manifold %d times.\r" % n
            sys.stdout.flush()
        self.pump_manifold(min_time=min_time, timeout=timeout)
        for i in range(n):
