...
class RabiSequence(PulseSequence):
    def __init__(self, awg_info, rabi_cfg, readout_cfg):
        ...
        self.max_length = round_samples((max_pulse_width + self.measurement_delay + self.measurement_width+1000)*1.2)
        self.origin = self.max_length/1.2 - (self.measurement_delay + self.measurement_width + 500)

        PulseSequence.__init__(self, "Rabi", awg_info, self.max_length, sequence_length)
    ...
    def build_sequence(self):
        ...
        for ii, d in enumerate(self.rabi_pts):
            ...
            self.waveforms['qubit drive I'][ii], self.waveforms['qubit drive Q'][ii] = \
                ap2.sideband(wtpts,
                             ap2.square(wtpts, a,self.origin - w - 2 * self.sigma, w, self.sigma), np.zeros(len(wtpts)),
                             self.freq, self.phase)

            self.markers['qubit buffer'][ii] = ap2.square(mtpts, 1, self.origin - w - 4 * self.sigma - 100,
                                                      w + 4 * self.sigma + 200)
