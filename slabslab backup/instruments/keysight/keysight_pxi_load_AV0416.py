...
class KeysightSingleQubit:
    ...
    def loadAndQueueWaveforms(self,sequences):
       ...
       # newname=s
        ...
        #print("generated some of the markers")
    ...
    def acquire_avg_data(self,w = [0,-1],pi_calibration=False,rotate_iq = False,phi=0):
        if rotate_iq:
            ...
            #REMOVED TQDM HERE
            # Reinserted by MGP December 11, 2019, hopefully correctly, removed April 1, 2020  - it's in tqdm(range...) i think
            # And reinserted September 17, 2020
            for ii in tqdm(range(self.num_avg)):
                id=self.DIG_ch_1.readDataQuiet(timeout=20000)
                qd=self.DIG_ch_2.readDataQuiet(timeout=20000)
                # id=self.DIG_ch_1.readData(data_points=self.data_1.shape[0]*self.data_1.shape[1])
                # qd = self.DIG_ch_2.readData(data_points=self.data_2.shape[0] * self.data_2.shape[1])

                Itemp = np.reshape(id, self.data_1.shape).T[int(w[0]):int(w[1])]
                #print("got thru 1")
                Qtemp = np.reshape(qd, self.data_2.shape).T[int(w[0]):int(w[1])]
