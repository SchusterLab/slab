...
class Bridge_with_SiOx(Structure):
    ...
    def siox_underneath(self, layer='siox block', color=1):
        ...
        self.structure.append(sdxf.PolyLine(pts, layer=layer))
    def hf_box(self, layer='hf_layer', color=1):

        width = self.pad_width + 8
        height = 2*self.pad_height + 6 + self.bridge_length
        # if self.direction == 0:
        startx = self.x_start - width / 2
        starty = self.y_start + height / 2
        pts = [(startx, starty), (startx + width, starty),
    ...
    def draw_bridge_with_siox(self, metal_layer='metal bridge', siox_layer='siox layer', hf_layer = 'hf_layer',
                              metal_color=1, siox_color=1, hf_color = 1):
        self.bridge(metal_layer, metal_color)
        self.siox_underneath(siox_layer, siox_color)
        self.hf_box(hf_layer, hf_color)
...
class CPWStraight:
    def __init__(self, structure, length, pinw=None, gapw=None, bridges=False, pos=500, ratio=3, offset=0,
