...
class WaferMask(sdxf.Drawing):
    ...
    def add_chip(self, chip, copies, label=False, savechip=True):
        ...
        if self.dashed_dicing_border > 0:
            dashlayer = 'gap' if chip.three_layer else chip.layer
        ...
        if chip.three_layer:
            self.layers.append(sdxf.Layer(name='gap', color=1))
            self.layers.append(sdxf.Layer(name='pin', color=3))
            self.layers.append(sdxf.Layer(name='port', color=5))
            self.blocks.append(chip.gap_layer)
            self.blocks.append(chip.pin_layer)
            self.blocks.append(chip.port_layer)
        ...
        for ii in range(copies):
            ...
            if chip.three_layer:
                self.append(sdxf.Insert(chip.name + 'gap', point=p, layer='gap'))
                self.append(sdxf.Insert(chip.name + 'pin', point=p, layer='pin'))
                self.append(sdxf.Insert(chip.name + 'port', point=p, layer='port'))
            if label:
...
class Chip(sdxf.Block):
    ...
    def __init__(self, name, author='', size=(7000., 1900.), mask_id_loc=(0, 1800), chip_id_loc=(0, 0),
                 author_loc=(6900, 100), textsize=(160, 160), two_layer=False, three_layer=False, layer=None, solid=False, do_save=True, **kwargs):
        """size is a tuple size=(xsize,ysize)"""
        name = name.upper()
        self.two_layer = two_layer
        self.three_layer = three_layer
        if two_layer:
            self.gap_layer = Chip(name + "gap", size, mask_id_loc, chip_id_loc, textsize, layer='gap', solid=solid)
            self.pin_layer = Chip(name + "pin", size, mask_id_loc, chip_id_loc, textsize, layer='pin', solid=solid)
        if three_layer:
            self.gap_layer = Chip(name + "gap", size, mask_id_loc, chip_id_loc, textsize, layer='gap', solid=solid)
            self.pin_layer = Chip(name + "pin", size, mask_id_loc, chip_id_loc, textsize, layer='pin', solid=solid)
            self.port_layer= Chip(name + "port", size, mask_id_loc, chip_id_loc, textsize, layer='port', solid=solid)
    ...
    def save(self, fname=None, maskid=None, chipid=None, do_label = True):
        ...
        if self.three_layer:
            ...
            d.layers.append(sdxf.Layer(name='port', color=3))
            ...
            d.blocks.append(self.port_layer)
            d.append(sdxf.Insert(self.port_layer.name, point=(0, 0), layer='port'))
            print((d.layers))
            print((self.gap_layer.layer))
            print((self.pin_layer.layer))
            print((self.port_layer.layer))
        elif self.two_layer:
            d.layers.append(sdxf.Layer(name='gap', color=1))
            d.layers.append(sdxf.Layer(name='pin', color=3))
            self.label_chip(self.gap_layer, maskid, chipid, self.author)
    ...
    def __init__(self, chip, start=(0, 0), direction=0, layer="structures", color=1,
        ...
        if chip.three_layer:
            ...
            self.pin_layer = Structure(chip.pin_layer, start, direction, 'pin',
                                       2, defaults)
            self.port_layer = Structure(chip.port_layer, start, direction, 'port',
                                       3, defaults)
        self.chip = chip
        self.start = start
        self.last = start
...
# class BoxShapeAlignmentMarks:
#     def __init__(self, structure, width, solid = True, layer='structure'):
#         """
#         Creates an L shaped alignment marker of width and armlength for photolithography.
#         """
#         if width == 0: return
...
#         ]
#         box1 = rotate_pts(box1, s.last_direction, start)
#         stop = rotate_pt((start[0], start[1]), s.last_direction, start)
#         s.last = stop
#
#         if solid:
#             s.append(sdxf.Solid(box1[:-1], layer=layer))
