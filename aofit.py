...
"""
Introduction to the lm_curve_fit function
...
See ipython notebook for tutorial
"""
...
def get_model_params(fit_func):
    if inspect.isclass(fit_func):
        ...
        if module_name == lmfit.model.__name__:
            print('Model is: %s' % str(module_name).split(': ')[1].rsplit('>')[0])
            param_dict = {}
        ...
        raise Exception('ERROR: fit function is %s, needs to be lm_fit model class or function.' %
                        str(type(fit_func)).split(' ')[1].rsplit('>')[0])
        ...
        if p_exprs != None:
            if type(p_exprs) == list:
                                    ...
                                    if val[key] != None:
                                        ...
                                        if count == 0:
                                            ...
                                            if inspect.isfunction(val[key]):
                                                ...
                                                raise Exception(
                                                    'Variable input must be function or list, type of input is %s' %
                ...
                raise Exception(
                    ...
                    str(type(p_exprs)).split(' ')[1].rsplit('>')[0])
