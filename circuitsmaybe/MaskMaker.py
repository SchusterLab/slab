...
class Bridge_with_SiOx(Structure):
    ...
    def __init__(self, structure, bridge_width, bridge_length, pad_width, pad_height, 
        ...
        '''f direction >= 315 or direction <= 45:
            self.direction = 0
    ...
    def bridge(self, layer = 'metal_bridge', color = 1):
        '''starting_x = self.x_start - self.bridge_length/2 - self.pad_height 
        starting_y = self.y_start + self.pad_width/2
        
        
        pts = [(starting_x, starting_y), (starting_x + self.pad_height, starting_y), 
               (starting_x + self.pad_height, starting_y - self.pad_width/2 + self.bridge_width/2), 
               (starting_x + self.pad_height + self.bridge_length, starting_y - self.pad_width/2 + self.bridge_width/2),
               (starting_x + self.pad_height + self.bridge_length, starting_y), 
               (starting_x + 2*self.pad_height + self.bridge_length, starting_y), 
               (starting_x + 2*self.pad_height + self.bridge_length, starting_y - self.pad_width), 
               (starting_x + self.pad_height + self.bridge_length, starting_y - self.pad_width), 
               (starting_x + self.pad_height + self.bridge_length, starting_y - self.bridge_width/2 - self.pad_width/2), 
               (starting_x + self.pad_height, starting_y - self.bridge_width/2 - self.pad_width/2), 
               (starting_x + self.pad_height, starting_y - self.pad_width), 
               (starting_x , starting_y - self.pad_width), 
               (starting_x, starting_y)]'''
    ...
    def siox_underneath(self, layer = 'siox block', color = 1):
        ...
        pts = [(startx, starty), (startx + width, starty), 
                  ...
                  (startx, starty)]
        #else:
        #    startx = self.x_start-height/2
        #    starty = self.y_start + width / 2
        #    pts = [(startx, starty), (startx + height, starty), 
        #               (startx + height, starty - width), (startx, starty - width),
        #               (startx, starty)]
...
class Bridge_Structure(Structure):
    ...
    """
    def __init__(self, structure, width = 2, bridge_length = 20, 
                 connection_width = 16, connection_height = 30, layer='metal_bridge', color=1):
            
        s = structure
        xstart, ystart = s.last
        ...
        pts1 = [(starting_y,starting_x), (starting_y, starting_x + connection_width), 
               ...
               (starting_y - connection_height, starting_x ), 
               (starting_y, starting_x)]
        #pts = [(2000, 2000), (2300, 2000), (2300, 2300), (2000, 2300), (2000, 2000)]
        #pts1 =[(2000, 2000), (2300, 2000), (2300, 2300), (2000, 2300), (2000, 2000)]
        
        
        if direction == 0:
            s.append(sdxf.PolyLine(pts1, layer=layer))
        else: 
            s.append(sdxf.PolyLine(pts, layer=layer))


class Box_Structure():
    def __init__(self, structure, width, height, layer='SiOx_block'):
        """
...
class BoxFreeStanding(Structure):
    """
    Dashed Chip border for e-beam drawing and then dicing
    

    """
    def __init__(self, chip, width = 100, height = 200, middle_bottom = (500, 600), offset_x = 2000, offset_y = 2000, layer='sio2 layer', color=1):
            Structure.__init__(self, chip, layer=layer, color=color)
...
class BoxNotCentered(Structure):
    """
    Dashed Chip border for e-beam drawing and then dicing
...
class CPWStraight:
    def __init__(self, structure, length, pinw=None, gapw=None, bridges = False, pos = 1500, offset = 0, insert_bridge = False ):
        s = structure
        if pinw is None: pinw = structure.__dict__['pinw']
        if gapw is None: gapw = structure.__dict__['gapw']
        pinw, gapw = float(pinw), float(gapw)
        ...
        if square:
            ...
            for ii in range(num_wiggles):
                ...
                if ii == 0:
                    ...
                    #    place_bridge = False
                    CPWStraight(s, vlength + offset, pinw, gapw, insert_bridge = place_bridge)
                CPWBend(s, isign * asign * 180, pinw, gapw, radius, segments=segments)
                if ii < num_wiggles - 1:
                    CPWStraight(s, 2 * vlength + 2 * radius, pinw, gapw, insert_bridge = place_bridge)
                else:
                    if vlength + offset < distance_bridge:
                        place_bridge = False
                    CPWStraight(s, vlength + offset, pinw, gapw, insert_bridge = place_bridge)
...
class SiEtchFingerCapV2:
    ...
    def outer_connections(self, structure):
        ...
        #structure, length, ratio=1., pinw=None, gapw=None,
        #         stop_pinw=None, stop_gapw=None
...
class CPWFingerCap:
    ...
    def __init__(self, num_fingers, finger_length, finger_width, finger_gap, taper_length=0, gapw=None, pinw = None,
        ...
        self.finger_gap = finger_gap
        self.gapw = gapw                    #gap between "center pin" and gnd planes        
        self.pinw = pinw    #effective center pin width sum of finger gaps and widths
        self.length = finger_length + finger_gap
        self.taper_length = taper_length
        self.total_length = finger_length + finger_gap + 2. * taper_length
...
class FineAlign:
    def __init__(self, chip, buffer=60, al=60, wid=2):
        ...
        beShapeAlignmentMarks(s1, width=wid, armlength=al, layer=layer)
